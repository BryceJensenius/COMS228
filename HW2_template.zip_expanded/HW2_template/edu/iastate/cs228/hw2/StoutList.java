...
public class StoutList<E extends Comparable<? super E>> extends AbstractSequentialList<E> {
	...
	/*
	 * Adds element to the stoutlist at the end if item is null, throws exception
	 * 
	...
	/*
	 * Add an element to the stoutlist at a specific index Illegal argument
	 * exception if not within the stoutlist size Null point exception if element is
	 * null
	 * 
	...
	public void sort() {
		...
		insertionSort(elem, new ElementComparator<E>());// sort with the comparator implemented

		// add all the elements in the sorted list back into StoutList, method from
	...
	/*
	 * Locates the item at the real index of pos returning the node and index in
	 * that node
	 * 
	...
	/*
	 * Adds item to index of the node Handles shifting to overflow elements
	 * 
	...
	private void addElement(E item, Node node, int index) {
		...
		if (index != 0) {
			if (node.count < nodeSize) {// still room in node so add at given index
				...
				// this node is now full so put split the elements between this and the second
				// node, keeping order
			...
			} else if (node.previous.count < nodeSize && node.previous != head) {// add to previous node if there is
																					// room left, unless previous is
																					// head
				node.previous.addItem(item);
			} else if (node == tail && node.previous.count == nodeSize) {// add new node at tail because previous is
																			// full and current is tail
				Node newNode = new Node();
				newNode.addItem(item);
				connectTwoNodes(tail.previous, newNode);// connect new note to after last node
			} else {
				// because of how find and add are implemented, there will always be room for
				// this element here
				node.addItem(index, item);
			}
		}
...
//      System.out.println("Added " + item.toString() + " at index " + offset + " to node: "  + Arrays.toString(data));
	...
	private class StoutListIterator implements ListIterator<E> {
		...
		/*
		 * Previous move direction 0 = none 1 = backward -1 = forward
		 */
		private int direction = 0;

		/**
		 * Default constructor, kept for testing
		 */
	...
	/*
	 * Given element info with node and index, remove that element from the
	 * stoutList
	 * 
	...
	private E remove(NodeInfo n) {
		...
		if (n.node.next == tail && n.node.count == 1) {// last node, one element, easy case of directly remove element
			unlink(n.node);
		} else if (n.node.count > nodeSize / 2 || n.node.next == tail) {// node is last node or has enough element to
																		// safely just remove
			...
			if (n.node.next.count > nodeSize / 2) {// node after node needing another element is big enough to have one
													// taken
